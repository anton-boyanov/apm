terraform {
  source = "${get_env("TG_ROOT_DIR", "/terragrunt")}/modules/aws//elasticache_subnet_group"
}
...
locals {
  ...
  tags = merge({
    ...
    },
    {
      ...
      Description = "external-dashboard subnet group",
    }
  )
}
...
inputs = {
  name       = "${local.common_vars.environment}_${local.common_vars.region_country_code}_main-redis-cluster"
