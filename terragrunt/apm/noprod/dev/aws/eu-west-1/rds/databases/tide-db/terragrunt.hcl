...
locals {
  ...
  tags = merge({
    ...
    },
    {
      ...
      Description = "tide-db RDS Cluster",
    }
  )
}
...
dependencies {
  paths = [
    ...
    "../../subnet_groups",
    "../../parameter_groups/tide-db_cluster-params",
    "../../parameter_groups/tide-db_db-params",
    "../../../vpc/security_groups/tide_db_sg"
  ]
}
...
dependency "cluster_params" {
  config_path = "../../parameter_groups/tide-db_cluster-params"
  mock_outputs = {
...
dependency "db_params" {
  config_path = "../../parameter_groups/tide-db_db-params"
  mock_outputs = {
    db_params_group_name = "fake-db_params_group_name"
  }
}

dependency "tide_db_sg" {
  config_path = "../../../vpc/security_groups/tide_db_sg"
  mock_outputs = {
    id = "fake-sg_id"
  }
}
...
inputs = {
  ...
  vpc_security_group_ids  = [dependency.tide_db_sg.outputs.id]
  db_subnet_group_name    = dependency.subnet_group.outputs.subnet_group_name
  backup_retention_period = local.common_vars.backup_retention_period.tide-db
  # SET prevent_destroy to TRUE IN FINAL VERSION !!!
  prevent_destroy              = false
  num_instances                = local.common_vars.num_instances.tide-db
  instance_type                = local.common_vars.instance_type.tide-db
  ...
  monitoring_interval          = local.common_vars.monitoring_interval.tide-db
