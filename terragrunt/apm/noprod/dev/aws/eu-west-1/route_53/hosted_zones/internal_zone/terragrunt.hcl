terraform {
  source = "${get_env("TG_ROOT_DIR", "/terragrunt")}/modules/aws//route53_zone"
}
...
locals {
  ...
  tags = merge({
    ...
    },
    {
      ...
      Description = "Public internal DNS zone",
    }
  )
}
...
inputs = {
  zone_name = "${local.common_vars.internal_zone_infix}.${local.common_vars.top_domains[local.common_vars.region_country_code][local.common_vars.environment]}."

  # Tags are required (when possible)
  tags = local.tags

  comment = local.tags.Description
}